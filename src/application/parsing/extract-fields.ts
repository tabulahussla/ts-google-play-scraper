...
export const MAPPINGS: any = {
	...
	updated: {
		path: ["ds:5", 0, 12, 8, 0],
		fun: (ts: number) => new Date(ts * 1000),
	},
 ...
 */
export function extractor(scriptData: any, request: any) {
	return (spec: any) => {
...
export function extractStoreId(storeUrl: string) {
...
export function cleanInt(value: string) {
	value = value || "0";
	value = value.replace(/[^\d]/g, ""); // removes thousands separator
	return parseInt(value, 10);
}

export function normalizeAndroidVersion(androidVersionText: string) {
	const value = androidVersionText.split(" ")[0];
	if (parseFloat(value)) {
		return value;
	}
	return "VARY";
...
export function buildHistogram(container: any) {
	if (!container) {
